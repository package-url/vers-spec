...
VERS stands for "VErsion range specifier. A VERS is a URI string composed of
three components:
...
A list of **version-constraints** is a signpost in the version
...
[MJH - need to get markdown bolding (asterisks) to work for each character definition above]
...
The **version-scheme** (such as "npm", "deb", etc.) determines:

- the specific notation and conventions used for a version string
  encoded in this scheme. version schemes often specify a version
...
By convention the version scheme **should** be the same as the
...
After the **version-scheme** and "/" there are one or more
...
- The URI scheme and version scheme are always lowercase as in
  `vers:npm`.
- The versions are case-sensitive, and a version scheme may specify
