...
| deb       | Debian and Ubuntu use their own native notation and are known for their use of "epochs" to disambiguate versions. Their native comparators are '<<', '<=', '=', '>=' and '>>'. When converting Debian ranges to VERS, tools should translate '<<' to '<' and '>>' to '>'. | https://www.debian.org/doc/debian-policy/ch-relationships.html |
